...
const closeViewModalButton = document.getElementById('closeViewModal'); // Specific ID for View Modal close button
...
const historyModal = document.getElementById('history-modal'); // New modal for history
const historyDetails = document.getElementById('history-details'); // Container for history details
const closeHistoryModalButton = document.getElementById('closeHistoryModal'); // Button to close history modal
...
let currentViewingStudentId = null; // To store the ID of the student whose details are being viewed
...
function renderStudents(data) {
    ...
    if (data.length > 0) {
        ...
        // If data is empty (e.g., search field is empty), do nothing to show nothing.
    }
}
...
function saveStudentData(newStudentData, studentId = null) {
    ...
    const modifiedBy = currentUser ? currentUser.uid : 'Unknown User'; // Get user ID or mark as unknown
    ...
    if (studentId) {
        return studentsRef.child(studentId).get().then(snapshot => {
            ...
            for (const key in newStudentData) {
                if (newStudentData.hasOwnProperty(key)) {
                    ...
                    if (key === 'enrollmentHistory' || key === 'misconductInstances') {
                        // Function to compare objects based on content (ignoring order of keys)
            ...
            if (Object.keys(changes).length > 0) {
                ...
                const historyEntry = { timestamp: timestamp, changes: changes, modifiedBy: modifiedBy }; // Include modifiedBy
...
addStudentForm.addEventListener('submit', function(e) {
    ...
    saveStudentData(newStudent, currentStudentId)
        .then(() => {
            ...
            addRecordModal.style.display = 'none'; // Hide the modal after saving
...
function editStudent(studentId) {
    ...
    addRecordModal.style.display = 'block'; // Show the modal for editing
...
function viewStudent(studentId) {
    currentViewingStudentId = studentId; // Store the ID of the viewed student
            ...
            if (historyData) {
                ...
                if (historyEntries.length > 0) {
                    historyEntries.forEach(entry => {
                        ...
                        const modifiedByUserId = entry.modifiedBy; // Get the user ID

                        // Fetch user information (e.g., display name)
                        firebase.auth().getUser(modifiedByUserId)
                            .then((userRecord) => {
                                const displayName = userRecord.displayName || 'User ID: ' + modifiedByUserId;
                                ...
                                if (hasChanges) {
                                    entryDiv.innerHTML = `<h4>Modified At: ${formattedTime}</h4><p><strong>Modified By:</strong> ${displayName}</p>${changesHTML}<hr>`;
                            ...
                            .catch((error) => {
                                ...
                                changesHTML += '</ul>';
                                entryDiv.innerHTML = `<h4>Modified At: ${formattedTime}</h4><p><strong>Modified By:</strong> User ID: ${modifiedByUserId} (Error fetching name)</p>${changesHTML}<hr>`;
                                historyDetails.appendChild(entryDiv);
                            });
                    });
...
searchInput.addEventListener('input', function() {
    ...
    console.log("Search Input Value:", searchTerm); // Added console log

    if (searchTerm.trim() === "") {
        renderStudents([]); // Render an empty array if search is empty
    } else {
...
// Close modal functionality for Add Record Modal
...
// Close modal functionality for View Modal
...
// Close modal functionality for History Modal
...
// New code to handle the Add Record button
addRecordButton.addEventListener('click', () => {
    addRecordModal.style.display = 'block';
    studentFormHeader.textContent = 'Add New Student'; // Set heading when adding new
    studentIdInput.value = ''; // Clear any existing student ID
    lrnInput.disabled = false; // Enable LRN for new entry
    clearForm(); // Optionally clear the form when Add Record is clicked
});
...
studentsRef.on('value', (snapshot) => {
    ...
    if (searchInput.value.trim() === "") {
        ...
        // If there's a search term, perform the search and render results
        const searchTerm = searchInput.value.toLowerCase();
        const results = allStudentsData.filter(studentData => {
            const student = studentData.val();
            return (student.lrn && student.lrn.toLowerCase().startsWith(searchTerm)) ||
